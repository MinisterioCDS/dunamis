...
btn.addEventListener("click", e => {
  ...
  if(!lastname.value) return alert("Completa todos los campos");
  if(!age.value) return alert("Completa todos los campos");
  if(!phone.value) return alert("Completa todos los campos");
  if(phone.value.length !== 10) return alert("El celular debe tener diez números");
  popup.classList.add('active');
  confe.classList.add('active');
  fetch("https://dunamis-back.onrender.com", {
    method: "POST",
    headers: {
      "Content-Type": "application/json"
    },
    body: JSON.stringify(newJoven)
  }).then(res => {
    if (!res.ok) {
